package airplay

// FairPlay hardcoded certificates from RPiPlay/UxPlay.
// These are used for the /fp-setup RTSP handshake that iOS requires
// when the features bitmask advertises FairPlay authentication.

// fairplayReplies contains 4 possible 142-byte server certificates.
// iOS selects one by sending a mode byte (0-3) in its first /fp-setup request.
var fairplayReplies = [4][142]byte{
	{
		,
	},
	{
		,
	},
	{
		,
	},
	{
		,
	},
}

// fairplayHeader is the 12-byte header for the second /fp-setup response.
var fairplayHeader = [12]byte{
	0x46, 0x50, 0x4c, 0x59, 0x03, 0x01, 0x04, 0x00, 0x00, 0x00, 0x00, 0x14,
}

// fairplaySetup handles the first /fp-setup request.
// iOS sends 16 bytes; byte 4 must be 0x03 (FairPlay version).
// Byte 14 is the mode (0-3) selecting which certificate to return.
// Returns the 142-byte certificate response.
func fairplaySetup(req []byte) ([]byte, bool) {
	if len(req) < 16 || req[4] != 0x03 {
		return nil, false
	}
	mode := int(req[14])
	if mode > 3 {
		return nil, false
	}
	reply := make([]byte, 142)
	copy(reply, fairplayReplies[mode][:])
	return reply, true
}

// fairplayHandshake handles the second /fp-setup request.
// iOS sends 164 bytes; byte 4 must be 0x03.
// Returns 32 bytes: 12-byte header + 20 bytes copied from request at offset 144.
func fairplayHandshake(req []byte) ([]byte, bool) {
	if len(req) < 164 || req[4] != 0x03 {
		return nil, false
	}
	resp := make([]byte, 32)
	copy(resp[:12], fairplayHeader[:])
	copy(resp[12:], req[144:164])
	return resp, true
}
