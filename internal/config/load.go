...
func loadCameras(db *sql.DB, cfg *Config) {
	rows, err := db.Query(
		`SELECT name, type, ip, user, pass, channel, stream, enabled, vision_prompt,
		        ...
		        COALESCE(vision_stream, ''), COALESCE(vision_width, 0), COALESCE(snap_method, ''), COALESCE(sort_order, 0), COALESCE(tts_mode, '') FROM cameras`,
	)
	if err != nil {
		return
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&cam.VisionStream, &cam.VisionWidth, &cam.SnapMethod, &cam.SortOrder, &cam.TTSMode,
		); err != nil {
			continue
		}
