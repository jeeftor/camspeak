...
func TestTTSBenchmarkUsesSavedCredentialsWithoutPlayback(t *testing.T) {
	...
	defer server.Close()
	_, err := database.Exec(
		`INSERT INTO tts_presets(name,endpoint,model,default_voice,api_key) VALUES(?,?,?,?,?)`,
		"benchmark",
		server.URL,
		"kokoro",
		"af_sky",
	...
	request := httptest.NewRequest(
		...
		strings.NewReader(
			`{"preset":"benchmark","mode":"streaming","text":"test","sample_rate":24000,"channels":1}`,
		),
	)
	...
	if response.Code != http.StatusOK ||
		!strings.Contains(response.Body.String(), "data:audio/wav;base64,") ||
		strings.Contains(response.Body.String(), "saved-secret") {
