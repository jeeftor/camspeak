...
// grabFrameFromStream uses ffmpeg to capture a single frame from a go2rtc RTSP
// stream. Returns the JPEG bytes. The go2rtcURL should be the base URL
// (e.g. "http://192.168.1.120:1984"); the stream name is the go2rtc stream
// (e.g. "frontyard_sub"). If maxWidth > 0, the frame is scaled to fit within
